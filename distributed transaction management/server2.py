...
#create account of user2
user2=Account(2222,25000)
...
peerserversocket.bind((HOST, PEERPORT2))
    ...
    while True:
        ...
        if(reply=="vote_request"):
            ...
            if(reply=="commit"):
                ...
                with lock:
                    user2.deposit(int(transamount.decode()))
                    choice=input("\nEnter your choice\n1.success\n2.failure\n")
                    if(choice=="1"):
                ...
                if(cordsocket.recv(1024).decode()=="transaction failed"):
                    ...
                    with lock:
                        user2.withdraw(int(transamount.decode()))
                else:
...
def handle_connection(client_socket,addr):
    ...
    if(data=="balance enquiry"):
        ...
        if int(amount.decode()) > user2.get_balance():
            status="Insufficient balance"
        else:
            with lock:
                user2.withdraw(int(amount.decode()))
        ...
        if int(amount.decode()) > user2.get_balance():
            ...
            peerclientsocket.connect((HOST,PEERPORT1))
            # peerclientsocket.sendall(receiver_ac_no)
            ...
            if(reply=="vote_request"):
                ...
                if(reply=="commit"):
                    ...
                    if(cordsocket.recv(1024).decode()=="transaction failed"):
                        print("Got transaction failure message from coordinator.Rolling back the transaction..")
                        with lock:
                            user2.deposit(int(amount.decode()))
